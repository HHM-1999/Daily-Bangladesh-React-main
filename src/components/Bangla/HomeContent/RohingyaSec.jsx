...
export default function RohingyaSec() {
    const [state, setState] = useState([])
    useEffect(() => {
        axios
            .get(`${process.env.REACT_APP_API_URL}json/file/generateSpecial10.json`)
            .then(({ data }) => {
                if (data.data) {
        ...
        <>
            <div className="DRohingyaSec">
                <div className="DRohingyaBanner">
                    <img className="img-fluid img100" src={process.env.REACT_APP_FONT_DOMAIN_URL + "media/common/New-Site-Banner-3.jpg"} width={304} height={40} alt="রোহিঙ্গা ইস্যু" title="রোহিঙ্গা ইস্যু" />
                </div>
                <div className="DRohingyaNews">
                    {state.map((nc) => {
                        return (
                            <div className="DRohingyaList" key={nc.ContentID}>
