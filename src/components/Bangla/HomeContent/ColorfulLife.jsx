...
export default function ColorfulLife() {
    const [state, setState] = useState([])
    useEffect(() => {
        axios
            .get(`${process.env.REACT_APP_API_URL}json/file/generateCategory31.json`)
            .then(({ data }) => {
    ...
    return (
        <>
            <div className="SPSecTitle"><Link to="/colorful-life" onClick={scrollTop}><h2><img src={process.env.REACT_APP_FONT_DOMAIN_URL + "media/common/Sign.png"} width={16} height={16} alt="Sign" title="Sign" /> সাতরং</h2></Link></div>
            <div className="DCatStyle3" id="colorfulLife">
                {state.map((nc) => {
                    return (
