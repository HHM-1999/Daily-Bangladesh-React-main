...
import { ForLazyLoaderImg, scrollTop, banglaDateConvetar } from '../../AllFunctions'
import FBpagePlugin from '../FBpagePlugin'
import ErrorPageBn from '../ErrorPageBn';
import SubCatLdjsonBn from './SubCatLdjsonBn';
...
export default function SubCategorySlug() {
    ...
    useEffect(() => {
        ...
        axios
            .get(`${process.env.REACT_APP_API_URL}sub-category-info/${subCatSlug}`)
            .then(({ data }) => {
                ...
                if (data.message === undefined) {
                    ...
                    axios
                        .get(`${process.env.REACT_APP_API_URL}sub-category-content/${data.subCategory.CategoryID}/${LeadNewsLimit}`)
                        .then(({ data }) => {
    ...
    const toggleButtonState = () => {
        ...
        // axios
        //     .get(`${process.env.REACT_APP_API_URL}sub-category-contents-and-more/${subCatSlug}/${limit}/${offset}`)
        //     .then(({ data }) => {
        //         setIsLoadingData(false)
                    ...
                    <div className="container">
                        ...
                        <div className="row">
                            <div className="col-sm-12">
                                <SubCatLdjsonBn CatNames={catName.CategoryName} CatNameSlug={catName.Slug} SubCatNames={subCatName.CategoryName} SubCatNameSlug={subCatName.Slug} />
                                ...
                                <h2 className="mvp-feat1-pop-head"><Link to={'/sub/' + subCatName.Slug} onClick={scrollTop}><span className="mvp-feat1-pop-head">{subCatName.CategoryName || <Skeleton count={1} />}</span></Link></h2>
                            </div>
                        </div>
                        <div className="row">
                            <div className="col-lg-9 col-sm-12 border-right-inner1">
                                <section className="SubCategoryList read-more">
                                    {isSkeletonLoading ? <>
                                        ...
                                        {subCatreadMore && subCatreadMore.map((nc, i) => {
                                            return (
                                                <div className="DSubCategoryTopNews" key={nc.ContentID}>
                                                    <Link to={"/" + catName.Slug + "/" + nc.ContentID} onClick={scrollTop}>
                                                        <div className="row">
                                                            <div className="col-lg-4 col-sm-4 col-5 videoIcon">
                                                                <picture><img src={process.env.REACT_APP_LAZYL_IMG} data-src={nc.ImageSmPath ? process.env.REACT_APP_IMG_Path + nc.ImageSmPath : process.env.REACT_APP_LAZYL_IMG} alt={nc.ContentHeading} title={nc.ContentHeading} className="img-fluid img100" /></picture>
                                                            ...
                                                            <div className="col-lg-8 col-sm-8 col-7">
                                                                ...
                                                                <span className="pDateAR"><i className="far fa-clock"></i> {banglaDateConvetar(nc.create_date)}</span>
                                                            </div>
                                                        </div>
                                                    </Link>
                                                </div>
