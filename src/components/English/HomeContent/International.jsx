...
export default function International() {
    ...
    useEffect(() => {
        axios
            .get(`${process.env.REACT_APP_API_URL_EN}json/file/generateCategory5.json`)
            .then(({ data }) => {
    ...
    return (
        <>
            <div className="SPSecTitle En"><Link to="/english/international" onClick={scrollTop}><h2><img src={process.env.REACT_APP_FONT_DOMAIN_URL + "media/common/Sign.png"} width={16} height={16} alt="Sign" title="Sign" /> International</h2></Link></div>
            <div className="DCatStyle2 En">
                <div className="DCatStyle2Top videoIcon">
                    <Link to={"/english/" + state.Slug + "/" + state.ContentID} onClick={scrollTop}>
                        <picture><img src={process.env.REACT_APP_LAZYL_IMG_En} data-src={process.env.REACT_APP_IMG_Path + state.ImageSmPath} alt={state.ContentHeading} title={state.ContentHeading} className="img-fluid img100" /></picture>
                ...
                <div className="DCatStyle2List">
                    {state2.map((nc) => {
                        return (
                            <div className="DCatStyle2ListItem" key={nc.ContentID}>
