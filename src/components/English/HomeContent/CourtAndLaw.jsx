...
export default function CourtAndLaw() {
    const [state, setState] = useState([])
    const [state2, setState2] = useState([])
    useEffect(() => {
        axios
            .get(`${process.env.REACT_APP_API_URL_EN}json/file/generateCategory4.json`)
            .then(({ data }) => {
    ...
    return (
        <>
            <div className="SPSecTitle En"><Link to="/english/law-and-court" onClick={scrollTop}><h2><img src={process.env.REACT_APP_FONT_DOMAIN_URL + "media/common/Sign.png"} width={16} height={16} alt="Sign" title="Sign" /> Law & Court </h2></Link></div>
